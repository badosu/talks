...
\begin{document}
  ...
  \begin{frame}{\LaTeX\ ?}
    Por quê \LaTeX?\\\pause
    \begin{itemize}
      \item{\fontfamily{cmr}\selectfont É necessário excelência tipográfica!\pause
        \begin{itemize}
  ...
  \begin{frame}[fragile]{Capacidades}
    ...
    \onslide<4>{$ \rightarrow $} É apenas texto\\\pause
    \onslide<5->{$ \rightarrow $} Expressões matemáticas\\\pause 
      $$ \oint_{D} B\cdot n\ dS = 0 $$
    \begin{center}
      \onslide<6->\pause\href{doc/rogers}{\beamerbutton{Exemplo}}\\
      \onslide<7->\pause\href{http://www.amazon.com/gp/product/1449314635}{\beamerbutton{Think Complexity}}
    \end{center}
\end{frame}
...
\end{frame}
  ...
  \begin{frame}{Motivação} 
    ...
    \onslide<2>{$ \nabla $} Quantidade enorme de tipos de documento\\\pause
    \onslide<3>{$ \nabla $} Aplicações web! Gerar documentos!\\
  \end{frame}
  \begin{frame}{Fim} 
